...
for i in auth_ids:
    for img in glob.glob(f'data/new_test/{i}/*.jpg'):
        try:
            dfs = DeepFace.find(img_path=img, db_path=autho, model_name='Facenet', silent=False)
            if dfs[0].shape[0] != 0:
                os.system(f'cp {img} data/{incoming_auth}')
                tested += 1
        except:
            print(f'Error with {img}')
...
for i in unauth_ids:
    for img in glob.glob(f'data/new_test/{i}/*.jpg'):
        try:
            dfs = DeepFace.find(img_path=img, db_path=autho, model_name='Facenet', silent=False)
            if dfs[0].shape[0] == 0:
                os.system(f'cp {img} data/{incoming_unauth}')
